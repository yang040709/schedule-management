...
<template>
  <div class="mt-4 max-w-[760px] mx-auto">
    ...
    <ul class="bg-white p-4 rounded-2xl w-full flex flex-col gap-2 mb-5">
      <li class="flex items-center gap-4 border-b border-gray-100 pb-4 last:border-b-0 last:pb-0"
        v-for="item in dataItems" :key="item.text">
        <div>
    ...
    <ul class="bg-white rounded-2xl w-full flex flex-col gap-2 mb-5">
      <li
        class="flex p-4 cursor-pointer transition-colors items-center gap-4 border-b border-gray-100 pb-4 last:border-b-0"
        v-for="item in aboutItems" :key="item.text" @click="$router.push({ name: item.link })">
        <div>
          <component :is="item.icon"></component>
        </div>
        <div class="flex justify-between w-full">
    ...
    <ul class="bg-white rounded-2xl w-full flex flex-col gap-2 mb-5">
      <li class=" p-4 cursor-pointer transition-colors  border-b border-gray-100 pb-4 last:border-b-0">
        ...
        <div class="mt-6">
          ...
          <div>
            ...
            <TagsList :tags="tagStore.tags" class="mt-4" :can-delete="true" @delete="tagStore.removeTag($event)" />
          </div>
        </div>
      </li>
    </ul>
