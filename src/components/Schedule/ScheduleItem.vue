...
<template>
  ...
  >
    ...
    <div class="relative">
      ...
      <!-- 依赖未完成提示 -->
      <div
        v-if="hasUncompletedDependency && item.status !== 'done'"
    ...
    <div class="min-w-0 flex-1">
      <div class="flex items-center gap-2 flex-wrap">
        <h4 class="truncate text-sm font-semibold text-slate-900 flex items-center">
      ...
      >
        ...
        <div class="space-y-2">
          ...
          <div class="space-y-2">
            <span class="text-sm font-medium text-red-600" v-if="item.status === 'locked'"
              >请先完成依赖任务，才能解锁任务 “{{ item.title }}”</span
            >
          </div>
          ...
          <div class="flex flex-wrap gap-2 text-xs">
            ...
            <span
              v-if="item.dependentSchedule.timeOfDay"
          ...
          <!-- 任务分类标签 -->
          <div
            v-if="item.dependentSchedule.category && item.dependentSchedule.category.length > 0"
            class="flex flex-wrap gap-1"
          >
            <span
              v-for="tag in item.dependentSchedule.category"
              :key="tag"
        ...
        <Popover>
          ...
          <PopoverContent class="w-auto">
            <div class="flex flex-col gap-3">
              ...
              <Dialog
                title="永久取消日程"
                desc="确定要永久取消这个日程吗？注意取消后无法恢复日程"
                @confirm="$emit('cancel', item.id)"
              />
              <Dialog
                title="删除日程"
                desc="确定要删除这个日程吗？"
